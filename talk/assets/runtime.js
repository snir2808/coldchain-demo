...
(function () {
  ...
  ready(function () {
    ...
    function openPresenterWindow() {
      ...
      if (!presenterWin) {
        alert('צריך לאשר חלונות קופצים בדפדפן כדי לפתוח את חלון המציג');
        return;
      }
...
<style>
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  /* Timer strip (lives in the bottom bar) */
  .bar-timer { display: inline-flex; align-items: baseline; gap: 12px; }
  .timer-display {
  ...
  .bar-count { font-size: 13px; color: #8b949e; font-weight: 600; }
  .bar-count .count-val { color: #e6edf3; font-family: "SF Mono", monospace; direction: ltr; unicode-bidi: isolate; }
  .bar-hints { color: #6e7681; }
  .timer-btn {
...
<div id="stage">
  ...
  <div class="pcard pcard-notes" id="card-notes" style="--dot-color:#f0883e">
    <div class="pcard-head" data-drag>
      <span class="pcard-dot"></span>
      <span class="pcard-title">תסריט מרצה · SPEAKER SCRIPT</span>
    </div>
...
<div class="hint-bar">
  <span class="bar-timer">
    ...
    <span class="bar-count">שקף <span class="count-val" id="timer-count">1 / ${total}</span></span>
  </span>
  <button class="timer-btn" id="btn-prev">הקודם</button>
  <button class="timer-btn" id="btn-next">הבא</button>
  <button class="timer-btn" id="btn-reset">איפוס טיימר</button>
  <span class="bar-hints"><kbd>← →</kbd> מעבר שקף · <kbd>R</kbd> איפוס · <kbd>Esc</kbd> סגירה · הזזת כרטיס בגרירת הכותרת, גודל בפינה</span>
  <button class="reset-layout" id="reset-layout">איפוס פריסה</button>
</div>
...
(function(){
  ...
  /* ===== Card layout as fractions of the stage =====
   * Positions/sizes are stored relative to the window, so the cards always
   * fill the available space and re-flow on every window resize. */
  function stageSize() {
    return { w: window.innerWidth, h: window.innerHeight - 48 }; /* hint bar */
  }
  function defaultLayout() {
    return {
      'card-cur':   { x: .012, y: .016, w: .505, h: .560 },
      'card-nxt':   { x: .012, y: .600, w: .505, h: .384 },
      'card-notes': { x: .530, y: .016, w: .458, h: .968 }
    };
  }
  var curLayout = null; /* current fractions; re-applied on resize */

  /* ===== Apply / save / restore layout ===== */
  function applyLayout(layout) {
    curLayout = layout;
    var s = stageSize();
    if (s.w < 50 || s.h < 50) return;
    Object.keys(layout).forEach(function(id){
      var el = document.getElementById(id);
      var l = layout[id];
      if (el && l) {
        el.style.left = Math.round(l.x * s.w) + 'px';
        el.style.top = Math.round(l.y * s.h) + 'px';
        el.style.width = Math.round(l.w * s.w) + 'px';
        el.style.height = Math.round(l.h * s.h) + 'px';
      }
    });
    rescaleAll();
  }
  function readLayout() {
    try {
      var saved = localStorage.getItem(STORAGE_KEY + ':v3');
      if (saved) {
        var l = JSON.parse(saved);
        /* sanity: fraction-based layouts only (w is 0..1) */
        if (l['card-cur'] && typeof l['card-cur'].w === 'number' && l['card-cur'].w <= 1) return l;
      }
    } catch(e) {}
  ...
  function saveLayout() {
    ...
    if (s.w < 50 || s.h < 50) return;
    var layout = {};
    ['card-cur','card-nxt','card-notes'].forEach(function(id){
      var el = document.getElementById(id);
      if (el) {
        layout[id] = {
          x: (parseInt(el.style.left,10) || 0) / s.w,
          y: (parseInt(el.style.top,10) || 0) / s.h,
          w: (parseInt(el.style.width,10) || 300) / s.w,
          h: (parseInt(el.style.height,10) || 200) / s.h
        };
      }
    });
    curLayout = layout;
    try { localStorage.setItem(STORAGE_KEY + ':v3', JSON.stringify(layout)); } catch(e) {}
  }
  ...
  window.addEventListener('resize', function(){
    if (curLayout) applyLayout(curLayout); else rescaleAll();
  });
  ...
  function update(n) {
    ...
    notesBody.innerHTML = note || '<span class="empty">(אין תסריט לשקף הזה)</span>';
  ...
  document.getElementById('reset-layout').addEventListener('click', function(){
    if (confirm('לאפס את פריסת הכרטיסים?')) {
      try { localStorage.removeItem(STORAGE_KEY); localStorage.removeItem(STORAGE_KEY + ':v2'); localStorage.removeItem(STORAGE_KEY + ':v3'); } catch(e){}
  ...
  notesBody.innerHTML = slideMeta[idx].notes || '<span class="empty">(אין תסריט לשקף הזה)</span>';
